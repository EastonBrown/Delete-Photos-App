...
- A full library enumeration is the *other* way to hear the library say "gone", and a scanning pool top-up already holds one. It uses it to retire collapsed Similar Groups and drop scan records for photos that no longer exist — questions about ids nobody is resolving, which per-photo lookups therefore never answer. Absence from that enumeration only counts when it covers the whole library: under limited access it is the user's chosen subset, and an empty enumeration is indistinguishable from a failed one, so both skip the reconciliation entirely rather than reading absence as deletion. Nothing there deletes a photo — the worst a wrong enumeration costs is a rehash and a group returned to one-by-one review — which is why it can act on absence where the per-photo path may not.
